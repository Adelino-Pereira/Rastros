...
describe('GridSizeSelector', () => {
  test.each(sizes.map(i => [i]))(
    ...
    (i) => {
      render(<GridSizeSelector rows={i} cols={i} onSelect={() => {}} disabled={false} />)
  ...
  test.each([
    ...
    [7, 9],
    [10, 6],
  ])('chama onSelect quando muda rows (%i) e mantém cols (%i)', (rows, cols) => {
    const onSelect = vi.fn()
    render(<GridSizeSelector rows={rows} cols={cols} onSelect={onSelect} disabled={false} />)

    const rowSlider = screen.getByRole('slider', { name: /altura/i })
    fireEvent.change(rowSlider, { target: { value: String(rows + 1 <= 11 ? rows + 1 : rows) } })
  ...
  test.each([
    ...
    const colSlider = screen.getByRole('slider', { name: /largura/i })
    fireEvent.change(colSlider, { target: { value: String(cols - 1 >= 5 ? cols - 1 : cols) } })
  ...
  test('respeita limites min/max (5–11)', () => {
    render(<GridSizeSelector rows={5} cols={11} onSelect={() => {}} disabled={false} />)
    const rowSlider = screen.getByRole('slider', { name: /altura/i })
    const colSlider = screen.getByRole('slider', { name: /largura/i })
