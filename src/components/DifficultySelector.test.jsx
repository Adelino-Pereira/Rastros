  ...
  describe('desenha o slider e vê se o número está correto', () => {
    for (let i = 1; i <= 10; i++) {
      test(`correto para o valor ${i}`, () => {
        render(<DifficultySelector value={i} onChange={() => {}} disabled={false} />)
...
//         validMoves={validMoves}
  ...
  test('chama onChange com o número quando se move o slider', () => {
    ...
    render(<DifficultySelector value={3} onChange={onChange} disabled={false} />)
    const slider = screen.getByRole('slider')
...
test('respeita os limites máximos (1..10)', () => {
  render(<DifficultySelector value={5} onChange={() => {}} disabled={false} />)
  const slider = screen.getByRole('slider')
